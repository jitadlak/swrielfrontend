...
const TABLE_HEAD = [
  { id: 'serviceproviderid', label: 'Service Provider ID', alignRight: false },
...
export default function ServiceProviderWithDrawRequest() {
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Service Provider WithDrawal Requests
          </Typography>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, amount, serviceprovider, approved } = row;
                    const selectedUser = selected.indexOf(serviceprovider) !== -1;

                    return (
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={serviceprovider}>
