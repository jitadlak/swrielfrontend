...
export default function AllVendors() {
    ...
    const updatebalance = async () => {
        try {
            ...
            setLoading(true)
            const res = await axios.patch('http://localhost:8000/user/updatewalletvendor', data);
            console.log(res, 'res');
            setLoading(false)
            if (res.data.status === 400) {
                alert(res.data?.message);
                    ...
                    <Scrollbar>
                        ...
                        <Modal open={open2} onClose={onCloseModal} center>
                            <Stack spacing={3}>
                                ...
                                <TextField name="ENTER UPDATED AMOUNT" label="ENTER UPDATED AMOUNT" onChange={(event) => setUpdatedAmount(event.target.value)} />
                                <TextField name="ENTER COMMENT" label="ENTER COMMENT" onChange={(event) => setComment(event.target.value)} />
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                                        const { _id, name, email, phone, address, membership, storeaddress, storecontactno, storename, walletBalance } = row;
                                        ...
                                        return (
                                            <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                                                ...
                                                <TableCell align="right">
                                                    ...
                                                    <MenuItem onClick={() => onOpenModal(_id)}>
