...
import { Grid, Container, Typography, CardHeader, Card, Tab, TableCell, TableRow, MenuItem, TextField } from '@mui/material';
                ...
                <Grid container spacing={4}>
                    <Grid item xs={12} md={6} lg={8}>
                        <Card>
                            ...
                            <Typography sx={{ mb: 1, ml: 3 }}>
                                Assigned To : {state?.assignTo}
                            </Typography>
                            {state.latitude && state.longitude ? <LoadingButton
                            ...
                            >
                                <a style={{ display: "table-cell", textDecoration: 'none', color: '#ffffff' }} href={`http://maps.google.com/?q=${state.latitude},${state.longitude}`} target="_blank"
                                    rel="noopener noreferrer">View Location On Map</a>
                            </LoadingButton> : null}
                            ...
                            <TableRow hover key={1} tabIndex={-1} role="checkbox" sx={{ ml: 5 }}>
                                <TableCell align="left">
                                    ...
                                    <Typography sx={{ mb: 1, ml: 3 }}>
                                        {state?.order.map((item, index) => {
                                            return <p>         Product ID  : {item._id}<br /></p>
                                        })}
                                    </Typography>

                                    <Typography sx={{ mb: 1, ml: 3 }}>
                                        {state?.order.map((item, index) => {
                                            return <p>   Product Name : {item.productName}<br /></p>
                                        })}
                                    </Typography>
                                ...
                                <TableCell align="left">
                                    <Typography sx={{ mb: 1, ml: 3 }}>
                                        {state?.order.map((item, index) => {
                                            return <p>   Product Title : {item.productTitle}<br /></p>
                                        })}
                                    </Typography>
                                ...
                                <TableCell align="left">
                                    <Typography sx={{ mb: 1, ml: 3 }}>
                                        {state?.order.map((item, index) => {
                                            return <p> Quantity :   {item.quantity}<br /></p>
                                        })}
                                    </Typography>
                                </TableCell>
                            </TableRow>
